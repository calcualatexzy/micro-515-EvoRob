...
class NSGAII(EA):
    ...
    def create_children(self, population_size: int) -> np.ndarray:
        ...
        if parent_pool_size < 3:
            raise ValueError(
                "NSGA-II requires at least 3 parents in the mating pool to create children."
            )
        ...
        for i in range(population_size):
            ...
            candidate_indices = np.delete(np.arange(parent_pool_size), parent_idx)
            r1, r2 = np.random.choice(candidate_indices, size=2, replace=False)
